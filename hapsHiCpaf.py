...
def showTrans(x, y):
    if np.isnan(x) or np.isnan(y):
        ...
        offsy = y - h2starts[i-1]        
        s ='X axis %s:%d Y axis %s:%d' % (chrx,offsx, chry, offsy)
    str_pane = pn.pane.Str(s, styles={'font-size': '10pt', 'color': 'darkblue', 'text-align': 'center'}, width=ptwidth)
    return str_pane
...
# and rotate so the diagonal becomes the x axis but need some kind of sideways scroller to work right
...
## --------------------cut here-----------------------------------------------------------------------------##
### once the pairs have been read and mapped into a grid, the code below does the plotting.
### it can be copied, edited to suit your needs and run repeatedly without waiting for the data to be mapped.
